...
FUNCTION RECGRABAR(oRecDiv)
  ...
  IF !MsgNoYes("Desea "+IF(lUpdate,"Actualizar","Crear")+" Recibo Número "+cNumero)
  ...
  IF lUpdate
    ...
    SQLUPDATE("DPDOCCLI","DOC_ACT",0,"DOC_CODSUC"+GetWhere("=",oRecDiv:cCodSuc)+" AND "+;
                                     "DOC_RECNUM"+GetWhere("=",cNumero        )+" AND "+;
                                     "DOC_TIPTRA"+GetWhere("=","P"            ))

    SQLUPDATE("DPCAJAMOV","CAJ_ACT",0,"CAJ_CODSUC"+GetWhere("=",oRecDiv:cCodSuc)+" AND "+;
...
//    EJECUTAR("DPDOCCLI_PRINT",oRecDiv:cCodSuc,oRecDiv:REC_TIPORG,oRecDiv:REC_NUMORG,cSerie,cImpFis)
  ...
  IF !oRecDiv:lCruce
    EJECUTAR("DPRECIBODIVBANCO",oRecDiv,oRecDiv:cCodSuc,aData,cNumero,oRecDiv:cCodigo,oRecDiv:cCodCaja,oRecDiv:dFecha,oRecDiv:cNomCli)
