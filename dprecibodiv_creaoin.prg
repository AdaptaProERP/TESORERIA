...
PROCE MAIN(cNumCbt,aData,cCodigo,dFecha,nValCam)
  LOCAL cNumDoc,I,oData,nNeto:=0,nIva:=0,nBruto:=0,nAt:=0
  LOCAL cTipDoc:="OIN",cNumero:="",cCenCos:=oDp:cCenCos,cTipIva:="",cSql
  ...
  IF !ISSQLFIND("DPTIPDOCCLI","TDC_TIPO"+GetWhere("=","OIN"))
     EJECUTAR("DPTIPDOCCLICREA","OIN","Otros Ingresos"     ,"D")
  ENDIF
  ...
  // JN 12/03/2015
  lPar_Zero:=.T. // SQLGET("DPTIPDOCCLI","TDC_ZERO","TDC_TIPO"+GetWhere("=",cTipDoc))
  nPar_Len :=10  // SQLGET("DPTIPDOCCLI","TDC_LEN" ,"TDC_TIPO"+GetWhere("=",cTipDoc))

// RViewArray(oDp:aDpIvaTabC)
// RETURN .T.

  IF !Empty(cNumCbt) .AND. SQLGET("DPDOCCLI","DOC_RECNUM,DOC_NUMERO",;
                                             "DOC_CODSUC"+GetWhere("=",cCodSuc)+" AND "+;
                                             "DOC_TIPDOC"+GetWhere("=",cTipDoc)+" AND "+;
                                             "DOC_RECNUM"+GetWhere("=",cNumCbt)+" AND "+;
  ...
  IF lAppend .OR. Empty(cNumero)
    ...
    oTable:Replace("DOC_RECNUM",cNumCbt      )
...
//  oTable:Replace("DOC_CODIGO",cCodigo      )
    ...
    oTable:Replace("DOC_CXC"   ,IIF(SQLGET("DPTIPDOCCLI","TDC_CXC","TDC_TIPO"+GetWhere("=",cTipDoc))="D",1,-1))
    oTable:Replace("DOC_ACT"   ,1            )
    oTable:Replace("DOC_VALCAM",nValCam      )

    oPago:=OpenTable("SELECT * FROM DPDOCCLI",.F.)
    oPago:Append()

    AEVAL(oPago:aFields,{|a,n|oPago:Replace(a[1],oTable:FieldGet(n))})
    oPago:Replace("DOC_TIPTRA","P")
    oPago:Replace("DOC_CXC"   ,-1 )
    oPago:Commit()
  ...
  ELSE
     ...
     cSql:="SELECT * FROM DPDOCCLI WHERE "+;
           "DOC_CODSUC"+GetWhere("=",cCodSuc  )+" AND "+;
           "DOC_TIPDOC"+GetWhere("=",cTipDoc  )+" AND "+;
           "DOC_RECNUM"+GetWhere("=",cNumCbt  )+" AND "+;
     ...
     SQLDELETE("DPDOCCLICTA","CCD_CODSUC"+GetWhere("=",cCodSuc)+" AND "+;
                             "CCD_TIPDOC"+GetWhere("=",cTipDoc)+" AND "+;
                             "CCD_NUMERO"+GetWhere("=",cNumero)+" AND "+;
                             "CCD_TIPTRA"+GetWhere("=","D"    ))


  //   SQLDELETE("DPDOCCLIIVA","IXD_TIPDOC"+GetWhere("=",oPago:DOC_TIPDOC)+" AND "+;
  ...
  //                           "IXD_TIPTRA"+GetWhere("=","D"))
     ...
     SQLUPDATE("DPDOCCLI","DOC_NETO",nNeto+nIva,"DOC_CODSUC"+GetWhere("=",cCodSuc  )+" AND "+;
                                                "DOC_TIPDOC"+GetWhere("=",cTipDoc  )+" AND "+;
                                                "DOC_RECNUM"+GetWhere("=",cNumCbt  )+" AND "+;
  ...
  FOR I=1 TO LEN(aData)
     ...
     oTable:=OpenTable("SELECT * FROM DPDOCCLICTA",.F.)
...
FUNCTION UPDATEDOC(lUpdate)
   ...
   cSql:="SELECT CCD_TIPIVA,CCD_PORIVA,SUM(CCD_MONTO) AS CCD_MONTO FROM DPDOCCLICTA WHERE "+;
...
*/  
   ...
   IF .F.
     ...
     cSql:="SELECT * FROM DPDOCCLIIVA WHERE "+;
